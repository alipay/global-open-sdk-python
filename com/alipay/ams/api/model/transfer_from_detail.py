...
class TransferFromDetail:
    ...
    def to_ams_dict(self):
        params = dict()
        if (
            hasattr(self, "transfer_from_method")
            and self.transfer_from_method is not None
        ):
            params["transferFromMethod"] = self.transfer_from_method
        if (
            hasattr(self, "transfer_from_amount")
            and self.transfer_from_amount is not None
        ):
    ...
    def parse_rsp_body(self, response_body):
        ...
        if "transferFromMethod" in response_body:
            ...
            self.__transfer_from_method.parse_rsp_body(
                response_body["transferFromMethod"]
            )
        if "transferFromAmount" in response_body:
            self.__transfer_from_amount = Amount()
            self.__transfer_from_amount.parse_rsp_body(
                response_body["transferFromAmount"]
            )
