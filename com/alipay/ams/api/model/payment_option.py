import json
from com.alipay.ams.api.model.payment_method_category_type import (
...
class PaymentOption:
    ...
    def to_ams_dict(self):
        ...
        if (
            hasattr(self, "payment_method_category")
            and self.payment_method_category is not None
        ):
        ...
        if (
            hasattr(self, "supported_currencies")
            and self.supported_currencies is not None
        ):
            params["supportedCurrencies"] = self.supported_currencies
        if (
            hasattr(self, "payment_option_detail")
            and self.payment_option_detail is not None
        ):
        ...
        if (
            hasattr(self, "amount_limit_info_map")
            and self.amount_limit_info_map is not None
        ):
    ...
    def parse_rsp_body(self, response_body):
        ...
        if "paymentMethodCategory" in response_body:
            payment_method_category_temp = PaymentMethodCategoryType.value_of(
                response_body["paymentMethodCategory"]
            )
        ...
        if "paymentOptionDetail" in response_body:
            ...
            self.__payment_option_detail.parse_rsp_body(
                response_body["paymentOptionDetail"]
            )
        ...
        if "interactionType" in response_body:
            interaction_type_temp = InteractionType.value_of(
                response_body["interactionType"]
            )
