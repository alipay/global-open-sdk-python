...
class PaymentFactor:
    ...
    def to_ams_dict(self):
        params = dict()
        if (
            hasattr(self, "is_payment_evaluation")
            and self.is_payment_evaluation is not None
        ):
            params["isPaymentEvaluation"] = self.is_payment_evaluation
        if (
            hasattr(self, "in_store_payment_scenario")
            and self.in_store_payment_scenario is not None
        ):
    ...
    def parse_rsp_body(self, response_body):
        ...
        if "inStorePaymentScenario" in response_body:
            in_store_payment_scenario_temp = InStorePaymentScenario.value_of(
                response_body["inStorePaymentScenario"]
            )
            self.__in_store_payment_scenario = in_store_payment_scenario_temp
        if "presentmentMode" in response_body:
            presentment_mode_temp = PresentmentMode.value_of(
                response_body["presentmentMode"]
            )
