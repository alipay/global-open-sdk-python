...
class Plan:
    ...
    def to_ams_dict(self):
        ...
        if (
            hasattr(self, "min_installment_amount")
            and self.min_installment_amount is not None
        ):
            params["minInstallmentAmount"] = self.min_installment_amount
        if (
            hasattr(self, "max_installment_amount")
            and self.max_installment_amount is not None
        ):
    ...
    def parse_rsp_body(self, response_body):
        ...
        if "minInstallmentAmount" in response_body:
            ...
            self.__min_installment_amount.parse_rsp_body(
                response_body["minInstallmentAmount"]
            )
        if "maxInstallmentAmount" in response_body:
            self.__max_installment_amount = Amount()
            self.__max_installment_amount.parse_rsp_body(
                response_body["maxInstallmentAmount"]
            )
