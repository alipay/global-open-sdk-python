...
class AlipayPriceInquireListResponse(AlipayResponse):
    ...
    def prices(self):
        """
        Price list. M - Always present; empty array &#x60;[]&#x60; when no results match. When prices array is non-empty, each Price object&#39;s M fields (priceId, productId, pricingModel, active, createdAt) are mandatory Returned only when result.resultCode is SUCCESS.
        """
    ...
    def total(self):
        """
        Total price count matching the query. O - Returned by default; absent only when includeTotal is explicitly set to false. When present, enables UI page navigation; when absent, use hasMore for \&quot;Load More\&quot; / infinite scroll patterns Returned only when result.resultCode is SUCCESS.
        """
        return self.__total
