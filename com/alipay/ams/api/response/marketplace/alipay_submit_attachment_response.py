...
class AlipaySubmitAttachmentResponse(AlipayResponse):
    ...
    def to_ams_dict(self):
        ...
        if (
            hasattr(self, "submit_attachment_request_id")
            and self.submit_attachment_request_id is not None
        ):
    ...
    def parse_rsp_body(self, response_body):
        response_body = super(AlipaySubmitAttachmentResponse, self).parse_rsp_body(
            response_body
        )
        ...
        if "submitAttachmentRequestId" in response_body:
            self.__submit_attachment_request_id = response_body[
                "submitAttachmentRequestId"
            ]
        if "attachmentType" in response_body:
            attachment_type_temp = AttachmentType.value_of(
                response_body["attachmentType"]
            )
